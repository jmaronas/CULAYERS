...
/////////////////////////////////////////////////////////////
gpu_env::gpu_env(int selected_gpu)//constructor
{
gpu_seed=1;
this->gpu_info(selected_gpu);
this->curandInit();
cublas.init();
...
void gpu_env::set_sc(float* gpu, float sc, tensor_gpu_specs* sp)
{
  if (gpu==NULL)
   ...
   {
     dimBlock.x=gpuI.maxThreadPerBlock;
     long int aux=(int)ceil(ops/(double)gpuI.maxThreadPerBlock);
     dimGrid.x=aux;  
   }

   tensor_set_value<<<dimGrid,dimBlock>>>(gpu,sc,ops);
   error_f();
...
{
  case ACT_RLU:
	ReLu<<<dimGrid,dimBlock>>>(E,N,ops);
	break;
  case ACT_ELU:
	ELU<<<dimGrid,dimBlock>>>(E,N,alfa,ops);
	break;
  case ACT_SOF:
       {
        ops=sp->row;
        ...
        set_sc(auxE, 0.0, sp);
	Softmax<<<dimGrid,dimBlock>>>(E,N,auxE,sample_dim,ops);
	break;
       }
  case ACT_SIG:
	Sigmoid<<<dimGrid,dimBlock>>>(E,N,ops);
	break;
  case ACT_LIN:
        //N=E;
...
{
  case ACT_RLU:
	dReLu<<<dimGrid,dimBlock>>>(E,D,ops);
	break;
  case ACT_ELU:
	dELU<<<dimGrid,dimBlock>>>(E,N,D,ops);
	break;
  case ACT_SIG:
	dSigmoid<<<dimGrid,dimBlock>>>(E,D,ops);
	break;
  case ACT_LIN:
       set_sc(D,1.0,sp);      
       break;
...
switch(t)
{
  case CE:
       ...
       error_f();
       MC_loss<<<dimGrid,dimBlock>>>(T,N,max_row,gsp->col,ops,cerr_g);
       error_f();
       ...
       error_f();
       CE_loss<<<dimGrid,dimBlock>>>(N,max_row,CE_vec,gsp->col,ops);
       cudaDeviceSynchronize(); 
       error_f();
       float* result_ce;
       error=cudaMalloc((void**)&result_ce,sizeof(float));
       error_f(); 
       reduce_array_sum<<<dimGrid,dimBlock,ops*sizeof(float)>>>(CE_vec,ops, gsp->col,result_ce);//carefull this might have a memory problem when launching big matrices. I did this only to have an example with shared memory
       cudaDeviceSynchronize(); 
       error_f();
       float aux1;
...
fprintf(stderr,"CULayers is running on GPU %s\n",prop.name);
...
if (op==0)
{
	...
	sc_add_mat<<<dimGrid,dimBlock>>>(vec_o,vec_i,sc,ops,acc);//could be same as matrix scalar operator because we treat as float*
...
if (op==0)
{
	...
	//add -> no blas operation (we could use same as product buth we should fill B in gpu and that is shit. Home made kernel)
	sc_add_mat<<<dimGrid,dimBlock>>>(mat_o,mat_i,sc,ops,acc);
...
void gpu_env::mat_elwise_mat(float* A, float* B, float* C,tensor_gpu_specs* sA,tensor_gpu_specs* sB,tensor_gpu_specs* sC,int op, int acc, int trA, int trB,float sca, float scb)
{
 ...
 dim3 dimGrid;
  ...
  else if(op==1) 
  {
	...
	if (trA==1 && trB==1){mat_ewprod_mat<<<dimGrid,dimBlock>>>(C,tA,tB,acc,ops);}
	if (trA==1 && trB==0){mat_ewprod_mat<<<dimGrid,dimBlock>>>(C,tA,B,acc,ops);}
	if (trA==0 && trB==1){mat_ewprod_mat<<<dimGrid,dimBlock>>>(C,A,tB,acc,ops);}
	if (trA==0 && trB==0){mat_ewprod_mat<<<dimGrid,dimBlock>>>(C,A,B,acc,ops);}
...
if (ops<=gpuI.maxThreadPerBlock)
  ...
  kern_tensor_equal<<<dimGrid,dimBlock>>>(A,B,ops);
  error=cudaDeviceSynchronize();
...
if (rdim==1){mat_ewcol_vec<<<dimGrid,dimBlock>>>(mat_o,mat,vec,ops,sA->col,op,acc,sca, scb);}
else{mat_ewrow_vec<<<dimGrid,dimBlock>>>(mat_o,mat,vec,ops,sA->row,op,acc,sca, scb);}
...
void gpu_env::mat_inplace_mat(float* o, float* i,tensor_gpu_specs* si,int op, int acc)
{
  ...
  if (op==0)//sqrt
  {tensor_sqrt<<<dimGrid,dimBlock>>>(o,i, acc,ops);}
  else
...
{
cublasHandle_t p_cublas=cublas.p_cublas;
...
void gpu_env::row_sum(float* A, tensor_gpu_specs* sA,float* B)
{
  ...
  tensor_set_value<<<dimGrid,dimBlock>>>(B,0.0,sA->row);
  error_f();
  error=cudaDeviceSynchronize();
  error_f();

  kernel_row_sum<<<dimGrid,dimBlock>>>(A,B,(int)sA->row,(int)sA->col,ops);
...
void gpu_env::col_sum(float* A, tensor_gpu_specs* sA,float* B)
{
  ...
  tensor_set_value<<<dimGrid,dimBlock>>>(B,0.0,sA->col);
  error_f();
  error=cudaDeviceSynchronize();
  error_f();

  kernel_col_sum<<<dimGrid,dimBlock>>>(A,B,(int)sA->row,(int)sA->col,ops);
...
void gpu_env::random_number_host_binary(float* rand_vec,tensor_gpu_specs* sp,float p)
{
  ...
  drop_mask<<<dimGrid,dimBlock>>>(rand_vec,p,ops);
  error_f(); 
  error=cudaDeviceSynchronize();
  error_f();
...
void gpu_env::curandInit()
{
  fprintf(stderr,"Initializing currand\n");
...
void gpu_env::set_seed(int seed)
{
   fprintf(stderr,"Setting new random seed\n");
   rand_error=curandSetPseudoRandomGeneratorSeed(random_generator,gpu_seed); 
    if (rand_error != CURAND_STATUS_SUCCESS)
	{fprintf(stderr,"Error seeting the seed for program\n");exit(-1);}
  fprintf(stderr,"sucess\n");
...
void gpu_env::add_noise(float* vec, float* rand_vec, float noiser, tensor_gpu_specs* sp)
{
  ...
  add_noise_with_mask<<<dimGrid,dimBlock>>>(vec,rand_vec,mask,noiser,ops);
  error_f(); 
  error=cudaDeviceSynchronize();
  error_f();
  destroyTensor(mask);
